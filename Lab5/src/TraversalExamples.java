...
public class TraversalExamples {
  ...
  // #region MODIFICATION BY JAYDEEP
  private static <E> Position<E> nextPreorderElementFromPos(BinaryTree<E> tree, Position<E> pos) {
    return traverseTree(tree, tree.root(), pos);
  }

  private static boolean isFoundPos;

  private static <E> Position<E> traverseTree(BinaryTree<E> tree, Position<E> pos, Position<E> target) {
    isFoundPos = pos == target;
    if (!tree.isEmpty() && pos != null) {
      // System.out.println(pos.getElement() + " found status is: " + isFoundPos);
    }
    if (tree.left(pos) != null) {
      if (isFoundPos) {
        // System.out.println("Found at " + pos.getElement() + " and next element in
        // preorder is " + tree.left(pos).getElement());
        return tree.left(pos);
      }
      traverseTree(tree, tree.left(pos), target);
    }
    if (tree.right(pos) != null) {
      if (isFoundPos) {
        // System.out.println("Found at " + pos.getElement() + " and next element in
        // preorder is " + tree.right(pos).getElement());
        return tree.right(pos);
      }
      traverseTree(tree, tree.right(pos), target);
    }

    return null;
  }

  private static <E> int findHeight(BinaryTree<E> tree, Position<E> pos) {
    ...
    int rightHeight = 0;
    if (tree.left(pos)!= null) {
      leftHeight = findHeight(tree, tree.left(pos));
    }
    if (tree.right(pos) != null) {
      rightHeight = findHeight(tree, tree.right(pos));
    }
  ...
  public static void main(String[] args) {
    ...
    if (nextPos == null) {
      System.out.println(target.getElement() + "'s next element is: " + nextPos);
    } else {
