...
class MergeSort {
  ...
  // #region bottom-up implementation
  public static <K> void merge(LinkedQueue<K> S, Comparator<K> comp) {
    LinkedQueue<LinkedQueue<K>> queues = new LinkedQueue<LinkedQueue<K>>();
    int n = S.size();
    for (int i = 0; i < n; i++) {
    ...
    while (queues.size() > 1) {
      Queue<K> S1 = queues.dequeue();
      Queue<K> S2 = queues.dequeue();
      LinkedQueue<K> merged = new LinkedQueue<K>();
